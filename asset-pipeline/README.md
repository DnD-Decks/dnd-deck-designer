...
2. Edit the issue's image-generation prompt in the workbench, then generate four low-quality brainstorming drafts in parallel. Each gets the same card concept and painterly style, but a distinct creative direction: isolated object or effect, defined actor, atmospheric silhouette, or unusual visual metaphor/viewpoint. Draft instructions permit omitting specific actors or settings from the scene. This replaces the old four modest composition changes. The image API has no temperature parameter; distinct instructions encourage divergence, but the model cannot guarantee four completely different results.
3. Automatically commit the completed drafts and a `run.json` containing the edited issue prompt and each exact candidate request prompt to the repository's default branch in `asset-pipeline/drafts/<issue-number>-<issue-title>/<run-id>/`. No PR or extra click is needed. A unique run directory preserves later attempts. The app retries a concurrent default-branch update by rebuilding on the newest tip; it never force-pushes. If GitHub rejects a write (for example, due to branch protection), the images remain in private R2 storage and the UI offers **Retry Git save**. Each draft opens in a full-screen viewer with fit-to-screen and 100% resolution modes.
4. Edit the selected draft once at high quality, using the prompt saved with that draft run, at the exact target aspect ratio. The selected visual concept takes precedence when original scene details were deliberately omitted. A visible spinner, disabled button, and elapsed-time display stay beside the final action while rendering, and the resulting PNG opens in the same viewer. Drafts use GPT Image 2.5 Flare at `low` quality in compressed JPEG, which favors speed and keeps the permanent Git history smaller. Previews are 720 × 1008 px or 1008 × 720 px. The final edit uses GPT Image 2 at `high` quality and outputs PNG at 800 × 1120 px or 1120 × 800 px. Both sizes match 5:7 or 7:5 exactly; final output exceeds the issue template's minimum dimensions. The 720 × 1008 previews satisfy the models' 655,360-pixel minimum; the previous 480 × 672 previews did not.
